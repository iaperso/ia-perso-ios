...
function minRelevance(promptWords) {
  return promptWords.length >= 4 ? 4 : 2;
}
...
function compactEnglishQuery(query) {
  const replacements = [
...
async function googleReferences(query, promptWords) {
  ...
  const minimum = minRelevance(promptWords);
  const candidates = (payload?.items || []).map((item, index) => {
    ...
    const relevance = score(context, promptWords);
    if (!/^https:\/\//i.test(landingUrl) || !/^https:\/\//i.test(thumbUrl) || relevance < minimum) return null;
    return {
...
async function openverseQuery(query, scoreWords) {
  ...
  const { response, payload } = await fetchJson(url, {
    headers: { Accept: 'application/json', 'User-Agent': 'IA-Perso/1.0 (visual reference search)' },
  });
  if (!response?.ok) return [];

  const minimum = minRelevance(scoreWords);
  return (payload?.results || []).map((item) => {
    ...
    if (relevance < minimum) return null;
    return {
...
async function openverseReferences(query, promptWords) {
  const english = compactEnglishQuery(query);
  const scoreWords = [...new Set([...promptWords, ...words(english)])];
  const primary = uniqueScored(await openverseQuery(query, scoreWords), WEB_RESULT_LIMIT);
  if (primary.length >= WEB_RESULT_LIMIT || !english) return primary;
  const translated = await openverseQuery(english, scoreWords);
  return uniqueScored([...primary, ...translated], WEB_RESULT_LIMIT);
}
...
async function commonsReferences(query, promptWords) {
  const english = compactEnglishQuery(query);
  const scoreWords = [...new Set([...promptWords, ...words(english)])];
  const primary = uniqueScored(await commonsQuery(query, scoreWords), WEB_RESULT_LIMIT);
  if (primary.length >= WEB_RESULT_LIMIT || !english) return primary;
  const translated = await commonsQuery(english, scoreWords);
  return uniqueScored([...primary, ...translated], WEB_RESULT_LIMIT);
}
...
async function blueskyQuery(query, promptWords) {
  ...
  for (const post of payload?.posts || []) {
    ...
    for (const image of [...direct, ...media]) {
      ...
      const relevance = score(`${postText} ${alt}`, promptWords);
...
async function blueskyReferences(query, promptWords) {
  const english = compactEnglishQuery(query);
  const scoreWords = [...new Set([...promptWords, ...words(english)])];
  const primary = uniqueScored(await blueskyQuery(query, scoreWords), BLUESKY_RESULT_LIMIT);
  if (primary.length >= BLUESKY_RESULT_LIMIT || !english) return primary;
  const translated = await blueskyQuery(english, scoreWords);
  return uniqueScored([...primary, ...translated], BLUESKY_RESULT_LIMIT);
}

export default async function handler(req, res) {
  if (req.method !== 'GET') return json(res, 405, { error: 'Méthode non autorisée.' });
  const query = clean(req.query?.q);
  if (!query) return json(res, 200, { results: [], web: [], google: [], openverse: [], commons: [], bluesky: [] });

  try {
    const promptWords = [...new Set(words(query))];
    const [openverse, commons, bluesky] = await Promise.all([
      openverseReferences(query, promptWords),
      commonsReferences(query, promptWords),
      blueskyReferences(query, promptWords),
    ]);

    const openWeb = uniqueScored([...openverse, ...commons], WEB_RESULT_LIMIT);
    let google = { results: [], configured: googleConfigured() };
    if (openWeb.length < WEB_RESULT_LIMIT && google.configured) {
      google = await googleReferences(query, promptWords);
    }

    const web = publicItems(uniqueScored([...openWeb, ...google.results], WEB_RESULT_LIMIT));
    ...
    return json(res, 200, {
      ...
      providers: ['openverse', 'wikimedia-commons', 'google-images-fr', 'bluesky-public'],
